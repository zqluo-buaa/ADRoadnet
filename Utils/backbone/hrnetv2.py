...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)  # 64 raw size
        out = self.bn1(out)  #64 raw size
        out = self.relu(out)  #64 raw size
        out = self.conv2(out)  #64 raw size
        out = self.bn2(out)#64 raw size
        out = self.relu(out)#64 raw size
        out = self.conv3(out)#256 raw size
        out = self.bn3(out)#256 raw size

        if self.downsample is not None:
            identity = self.downsample(x)  #256 raw size

        out += identity  # 256rawsize
        out = self.relu(out)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=1, padding=1, bias=False)  # why
        self.bn2 = nn.BatchNorm2d(planes)
        self.downsample = downsample
...
class StageModule(nn.Module):
    def __init__(self, stage, output_branches, c):
        ...
        # Note: Resolution + Number of channels maintains the same throughout respective branch. 在各自的分支结构
        for i in range(self.number_of_branches):  # Stage scales with the number of branches. Ex: Stage 2 -> 2 branch
            channels = c * (2 ** i)  # Scale channels by 2x for branch with lower resolution.  Lower convolution, more channels

            # Paper does x4 basic block for each forward sequence in each branch (x4 basic block considered as a block)
            branch = nn.Sequential(*[BasicBlock(channels, channels) for _ in range(4)])  # conduct conv+bn+relu
        ...
        for branch_output_number in range(self.output_branches):  # full connection contains two section--input and output, this cycle is for each output_branch

            self.fuse_layers.append(nn.ModuleList())  # for each output_branch, there is a list of options in all inputs.

            for branch_number in range(self.number_of_branches):  # this moment have number_of_branches branches
                if branch_number == branch_output_number:  # if they are equipment, their resolution are the same.
                    self.fuse_layers[-1].append(nn.Sequential())  # Used in place of "None" because it is callable. NO_OPITION
                elif branch_number > branch_output_number:  # need increase resolution
                    ...
                    ))  # conv+bn, the input_channel and out_channel size are computed by c and current stage.the scale_factor in Upsample is 2**(branch_number -                                                   branch_output_number)
                    ...
                    for _ in range(branch_output_number - branch_number - 1):
                        ...
                        ))  # use stride=2 downsample (branch_output_number - branch_number-1) times
                    downsampling_fusion.append(nn.Sequential(
    ...
    def forward(self, x):
        ...
        x = [branch(branch_input) for branch, branch_input in zip(self.branches, x)]  # in branches there are four basicconv to each stage
        ...
        for branch_output_index in range(
            ...
            for input_index in range(self.number_of_branches):  # The inputs of other branches to be fused.
                if input_index == 0:
                    ...
                    x_fused[branch_output_index] = x_fused[branch_output_index] \
                                                   + self.fuse_layers[branch_output_index][input_index](x[input_index])  # fuse all input branches, obtain a list of out branches
...
class HRNet(nn.Module):
    def __init__(self, c=48, num_blocks=[1, 4, 3], num_classes=1000):
        ...
        self.relu = nn.ReLU(inplace=True)  # domsample 4 times
        ...
        self.stage2 = nn.Sequential(
            *[StageModule(stage=2, output_branches=2, c=c) for _ in range(number_blocks_stage2)])  # in each stage, full connection in branches only conducted in the same                                                                                                          stage, the futher downsample is conducted below
    ...
    @staticmethod  # 静态方法无需实例化也可以调用，实例化也可以
    ...
    def forward(self, x):
        ...
        # Stage 2
        x = self.stage2(x)
        x.append(self.transition2(x[-1]))  # append the next stage feature

        # Stage 3
        x = self.stage3(x)
        x.append(self.transition3(x[-1]))  # actually, in this implement there is only one branch used to generate the new branch.
...
if __name__ == '__main__':
    ...
    try:
        CKPT_PATH = os.path.join(os.path.abspath(".."), 'checkpoints/model_best_epoch96_edit.pth')
